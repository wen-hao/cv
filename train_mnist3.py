...
# b_loader = LoadDataset('mnist', data_root, batch_size, 'train', style=domain_a)
...
while global_step < trainer_conf['total_step']:
    for i, (a_img, a_label) in enumerate(a_loader):

        # data augmentation
        # print(a_img[0][0].type(torch.FloatTensor).size())
        input_img = torch.FloatTensor(a_img)
        input_img = Variable(input_img, requires_grad=False)
        # print(input_img.size())
        img_label = Variable(a_label, requires_grad=False)
